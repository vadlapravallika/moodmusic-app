...
const WhackAMood = () => {
  ...
  // Update active mood in random slots
  useEffect(() => {
    let timer;
    if (gameActive) {
      timer = setInterval(() => {
  ...
  const handleWhack = (index) => {
    if (slots[index]) {
      ...
      newSlots[index] = null; // Clear slot after hit
      setSlots(newSlots);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-800 to-blue-600 flex flex-col items-center justify-center text-white p-4">
      {/* Back Button */}
      <button
